...
const homeCards = [
  { id: 'dictionary', title: 'Sambal Tina', body: 'Learn words, stories and culture', icon: BookOpen, tone: 'orange' },
...
function getMasinlocHour() {
  return Number(new Intl.DateTimeFormat('en-PH', { timeZone: MASINLOC_TIME_ZONE, hour: '2-digit', hourCycle: 'h23' }).format(new Date()));
}
...
function useMasinlocGreeting() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setGreeting(getMasinlocGreeting()), 60_000);
...
function useMasinlocWeather() {
  ...
  useEffect(() => {
    ...
    fetch(WEATHER_ENDPOINT(MASINLOC_CENTER), { signal: controller.signal, cache: 'no-store' })
      ...
      .then((data) => setWeather({ state: 'ready', temperature: Math.round(data.current.temperature_2m), condition: weatherLabels[data.current.weather_code] || 'Current Weather' }))
      .catch((error) => { if (error.name !== 'AbortError') setWeather({ state: 'unavailable' }); });
...
export function HomeHub({ navigate }) {
  ...
  const searchResults = useMemo(() => {
    ...
    const extras = [
      ...
      { id: 'report', title: 'Help Desk', body: 'PNP / MDRRMO incident and assistance reporting' },
...
export function MoreScreen({ navigate }) {
  return <div className="screen-stack"><ScreenTitle title="More Services" subtitle="Contribute history, words and community knowledge, and access other Masinloc Connect services." /><section className="more-menu-grid" aria-label="More services">{moreItems.map((item) => <MenuCard compact key={`${item.id}-${item.label}`} item={item} onClick={() => navigate(item.id)} />)}</section></div>;
}
