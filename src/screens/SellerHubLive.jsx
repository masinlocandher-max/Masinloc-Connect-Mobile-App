...
export default function SellerHub({ navigate }) {
  ...
  return <div className="screen-stack mobile-native-stack seller-mobile-hub">
    ...
    {submitted ? <div className="native-message"><Clock3 size={18}/><span>Submission {saved.reference_code} is in the review queue. Your listing is not automatically published.</span></div> : hasDraft ? <div className="native-message"><Save size={18}/><span>You have a business listing draft saved on this device.</span></div> : null}
...
export function SellerGuidelinesScreen() {
  return <div className="screen-stack mobile-native-stack"><ScreenTitle title="Business Listing Guidelines" subtitle="How business listings work inside Masinloc Connect." /><div className="policy-list native-policy-list"><article><Store size={22}/><div><h2>Marketplace listing</h2><p>Business information enters a review queue. Submission does not mean approval or automatic publication.</p></div></article><article><Check size={22}/><div><h2>Keep information accurate</h2><p>Use a real business name, usable location, current contact information and a working business page. Reviewers may need to verify details before publication.</p></div></article><article><Wrench size={22}/><div><h2>POS and order operations</h2><p>Masinloc POS is maintained separately and is still being completed. Masinloc Connect does not present unfinished POS or seller-order features as available.</p></div></article></div></div>;
}
